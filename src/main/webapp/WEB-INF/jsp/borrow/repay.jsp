...
<body>
			...
			<div class="col-sm-3 col-md-2 sidebar">
				<div class="tree">
					<ul style="padding-left: 0px;" class="list-group">
						...
						<li class="list-group-item "><span><i class="glyphicon glyphicon-th-large"></i> 业务管理 <span class="badge" style="float: right">2</span></span>
							<ul style="margin-top: 10px; ">
								<li style="height: 30px;"><a href="${APP_PATH}/borrow/bookBorrow" ><i class="glyphicon glyphicon-picture"></i> 借阅者名单 </a></li>
								<li style="height: 30px;"><a href="${APP_PATH}/borrow/repay" style="color: red;"><i class="glyphicon glyphicon-equalizer"></i> 逾期查看</a></li>
							</ul></li>
					</ul>
				</div>
			</div>
							...
							<table class="table  table-bordered">
								<thead>
									<tr>
										...
										<th width="120">罚金</th>
										<th width="100">处理日期</th>
										<th width="100">处理人员编号</th>
										<th width="100">备注</th>
									</tr>
								</thead>
	...
	<script type="text/javascript">
		...
		function pageQuery(pageNo) {
			...
			$.ajax({
				type : "POST",
				url : "${APP_PATH}/borrow/repay/pageQuery",
				data : jsonData,
				beforeSend : function() {
					index = layer.load(2, {
						time : 10 * 1000
				...
				success : function(result) {
					...
					if (result.success) {
						...
						for (var i = 0; i < datas.length; i++) {
							...
							context += '	<td>' + data.fine + '</td>';
							context += '	<td>' + data.repayDate+ '</td>';
							context += '	<td>' + data.manageId + '</td>';
							context += '	<td>' + data.remark + '</td>';
							context += '</tr>';
						}
