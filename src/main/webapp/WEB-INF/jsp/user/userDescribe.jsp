					...
					<ul style="padding-left: 0px;" class="list-group">
						<li class="list-group-item"><span><i class="glyphicon glyphicon glyphicon-tasks"></i> 权限管理 <span class="badge" style="float: right">2</span></span>
							<ul style="margin-top: 10px;">
								...
								<li style="height: 30px;"><a href="${APP_PATH}/information/userDescribe"  style="color: red;"><i class="glyphicon glyphicon-king"></i> 信息维护</a></li>
							</ul></li>
	...
	<script type="text/javascript">
		...
		function pageQuery(pageNo) {
			...
			$
					.ajax({
						type : "POST",
						url : "${APP_PATH}/information/userDescribe/pageQuery",
						data : jsonData,
						beforeSend : function() {
							index = layer.load(2, {
								time : 10 * 1000
						...
						success : function(result) {
							...
							if (result.success) {
								...
								for (var i = 0; i < datas.length; i++) {
									...
									context += '	<td>' + data.userSex + '</td>';
									context += '	<td>' + data.userMajor + '</td>';
									context += '	<td>' + data.userPhone + '</td>';
									context += '	<td>' + data.userEmial + '</td>';
									context += '	<td>' + data.userLocation + '</td>';
									context += '	<td>';
